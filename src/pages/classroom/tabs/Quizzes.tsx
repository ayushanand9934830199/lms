...
import { Plus, X, Clock, AlertTriangle, CheckCircle, ShieldAlert } from 'lucide-react';
...
const MOCK_QUIZZES: Quiz[] = [
    {
        id: 1,
        title: 'Midterm — Behavioural Economics',
        description: 'Covers Weeks 1–4. Duration 20 minutes. Tab switching is monitored.',
        durationMins: 20,
        maxTabSwitches: 2,
        published: true,
        questions: [
            { id: 1, question: 'Which bias describes our tendency to rely heavily on the first piece of information encountered?', type: 'mcq', options: ['Confirmation Bias', 'Anchoring Bias', 'Recency Bias', 'Availability Heuristic'], points: 2 },
            { id: 2, question: 'What does the concept of "loss aversion" mean in prospect theory?', type: 'mcq', options: ['Losses feel roughly equal to gains of the same size', 'Losses feel twice as painful as equivalent gains feel good', 'People avoid all risky decisions', 'People prefer guaranteed losses over possible larger losses'], points: 2 },
            { id: 3, question: 'Briefly explain the "endowment effect" and give one real-world example.', type: 'short', points: 5 },
        ],
    },
];
...
const QuizRunner: React.FC<{ quiz: Quiz; onExit: () => void }> = ({ quiz, onExit }) => {
    ...
    const [secsLeft, setSecsLeft] = useState(quiz.durationMins * 60);
    ...
    const handleVisibility = useCallback(() => {
        if (document.hidden && status === 'running') {
            setViolations(v => {
                const next = v + 1;
                if (next >= quiz.maxTabSwitches) {
                    setStatus('terminated');
                    stopTimer();
                } else {
                    setWarning(`⚠️ Warning ${next}/${quiz.maxTabSwitches}: You switched tabs. One more will auto-submit your quiz.`);
                    setTimeout(() => setWarning(null), 5000);
                }
                return next;
            });
        }
    }, [status, quiz.maxTabSwitches]);
    ...
    const submitQuiz = () => { stopTimer(); setStatus('submitted'); };
            ...
            <div style={{ maxWidth: 560 }}>
                <div className="card" style={{ marginBottom: '1.25rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.875rem', marginBottom: '1.5rem' }}>
                        {[['Questions', quiz.questions.length], ['Duration', `${quiz.durationMins} min`], ['Total Marks', totalPts]].map(([l, v]) => (
                    ...
                    <div style={{ background: '#FEF3C7', border: '1px solid var(--accent)', borderRadius: 'var(--radius-sm)', padding: '0.875rem 1rem', marginBottom: '1.5rem' }}>
                        ...
                        <p style={{ fontSize: '0.8125rem', color: '#78350F', lineHeight: 1.55 }}>
                            ...
                            You are allowed <strong>{quiz.maxTabSwitches}</strong> tab switch{quiz.maxTabSwitches !== 1 ? 'es' : ''} — after that, your quiz will be <strong>automatically submitted</strong> and flagged.
                        </p>
                    </div>
            ...
            <div style={{ maxWidth: 500 }}>
                <div className="card" style={{ textAlign: 'center', padding: '2.5rem' }}>
                    ...
                    <p style={{ color: 'var(--text-secondary)', lineHeight: 1.65, marginBottom: '1.5rem' }}>
                        Your quiz has been <strong>automatically submitted</strong> because you exceeded the allowed tab switches ({quiz.maxTabSwitches}).
    ...
    return (
        <div style={{ maxWidth: 680 }}>
            ...
            <div className="card" style={{ marginBottom: '1.25rem' }}>
                        ...
                        {q.options.map((opt, i) => (
                            <button
                                ...
                                style={{
                                    ...
                                    border: `1px solid ${answers[q.id] === opt ? 'var(--accent)' : 'var(--border-light)'}`,
                                    borderRadius: 'var(--radius-sm)',
                                    background: answers[q.id] === opt ? 'var(--accent-light)' : '#fff',
                                    cursor: 'pointer',
                                    textAlign: 'left',
                            ...
                            >
                                <span style={{ fontWeight: 600, marginRight: '0.625rem', color: 'var(--text-muted)' }}>
                ...
                {q.type === 'short' && (
                    <textarea className="input" style={{ minHeight: 130 }} placeholder="Write your answer here…"
...
/* ─────────────────────── Quiz List + Builder ─────────────────────── */
const ClassroomQuizzes: React.FC<{ classroom: Classroom; role: string }> = ({ role }) => {
    ...
    const createQuiz = () => {
        if (!bTitle.trim() || bQs.length === 0) return;
        setQuizzes(qs => [...qs, { id: Date.now(), title: bTitle, description: bDesc, durationMins: bDur, maxTabSwitches: bMax, published: false, questions: bQs }]);
        setCreating(false);
                    ...
                    <div className="card" style={{ width: 640, position: 'relative' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem', marginBottom: '1.25rem' }}>
                            ...
                            <div className="field" style={{ marginBottom: 0 }}>
                                <label className="label">Max Tab Switches (before auto-terminate)</label>
                    ...
                    <div key={q.id} className="card flex items-center justify-between">
                        <div>
                            ...
                            <div className="flex items-center gap-3" style={{ color: 'var(--text-muted)', fontSize: '0.8125rem' }}>
                                <span className="flex items-center gap-1"><Clock size={12} /> {q.durationMins} min</span>
                                <span>{q.questions.length} questions</span>
                                <span className="flex items-center gap-1"><ShieldAlert size={12} /> max {q.maxTabSwitches} tab switch{q.maxTabSwitches !== 1 ? 'es' : ''}</span>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            ...
                            {role === 'teacher' && (
                                <button className="btn btn-secondary" onClick={() => q.published = !q.published || true}>
                                    {q.published ? 'View Results' : 'Publish'}
                                </button>
                            )}
                        </div>
                    </div>
